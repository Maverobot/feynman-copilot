...
Also save the plan with writing to a file on disk so it survives context truncation.
...
Launch parallel researcher subagents via the `task` tool (agent_type: "general-purpose"). Each gets a structured brief with:
...
When the research includes quantitative data (benchmarks, performance comparisons, trends), generate charts using Mermaid diagram syntax in markdown. Use Mermaid diagrams for architectures and processes. Every visual must have a caption and reference the underlying data.
...
- Launch the full workflow via the `task` tool with mode: "background"
- Report the async ID and how to check status with the `read_agent` tool
